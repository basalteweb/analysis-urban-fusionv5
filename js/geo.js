...
AU.geo = (() => {
  ...
  function analyze(model){
    ...
    for(const z of zones.slice(0,8)){
      if(z.impactScore>=25){
        const loss=z.caGap<0?Math.abs(z.caGap):0;
        const facts=[`${z.current.visits} visites actuellement contre ${z.previous.visits} précédemment.`,`CA ${U().money(z.current.ca)} contre ${U().money(z.previous.ca)}.`,`Décomposition locale : effet visites ${z.trafficEffect>=0?'+':''}${U().money(z.trafficEffect)} ; effet panier ${z.basketEffect>=0?'+':''}${U().money(z.basketEffect)}. Driver dominant : ${z.driver}.`,`${z.highRisk} client(s) à risque élevé ; ${U().money(z.valueAtRisk)}/mois de valeur historique estimée.`];
        if(z.changePoint) facts.push(`Rupture de tendance détectée autour du ${U().formatDate(z.changePoint.date)} (${(z.changePoint.delta*100).toFixed(1)} % sur la moyenne hebdomadaire).`);
        if(z.hourShift&&Math.abs(z.hourShift.delta)>=0.08) facts.push(`Déplacement horaire marqué : ${z.hourShift.label} ${z.hourShift.delta>=0?'+':''}${(z.hourShift.delta*100).toFixed(1)} points de part des visites.`);
        findings.push({id:`geo-${n(z.name).replace(/\s+/g,'-')}`,category:'geo',level:z.impactScore>=70?'critical':'warning',title:`Pression géographique : ${z.name}`,summary:`CA ${z.caDelta===null?'non comparable':`${z.caDelta>=0?'+':''}${(z.caDelta*100).toFixed(1)} %`} ; visites ${z.visitsDelta===null?'—':`${z.visitsDelta>=0?'+':''}${(z.visitsDelta*100).toFixed(1)} %`}. Écart au reste de la clientèle : ${z.excess===null?'—':`${z.excess>=0?'+':''}${(z.excess*100).toFixed(1)} points`}.`,explanation:`Le moteur compare la zone à une période de même durée puis au reste de la clientèle. Un écart négatif concentré géographiquement est compatible avec un facteur d’accès ou de mobilité, sans constituer à lui seul une preuve causale.`,impactAmount:loss?-loss:0,confidence:clamp(55+z.impactScore/2,55,94),quality:'estimate',facts,hypotheses:z.excess!==null&&z.excess<-0.1?[`Le décrochage est plus fort que le marché interne de référence ; un facteur géographique local est plausible.`]:[],actions:[`Suivre automatiquement ${z.name} à chaque nouvel import.`,`Prioriser la liste des ${z.atRisk} clients en retard de revisite dans cette zone.`],entities:[z.name]});
      }
    }
    const positive=zones.filter(z=>z.caDelta!==null&&z.caDelta>0.12&&z.current.ca>300).sort((a,b)=>b.caDelta-a.caDelta)[0];
    if(positive) findings.push({id:'geo-opportunity',category:'geo',level:'opportunity',title:`Zone en accélération : ${positive.name}`,summary:`CA +${(positive.caDelta*100).toFixed(1)} % sur la fenêtre récente, avec ${positive.current.visits} visites.`,explanation:'La zone progresse davantage que sa période de référence. Le moteur la conserve comme zone témoin/opportunité pour les comparaisons futures.',impactAmount:positive.current.ca-positive.previous.ca,confidence:82,quality:'calculated',facts:[`Panier moyen ${U().money(positive.current.avgBasket)}.`,`Part du CA historique : ${(positive.historicalCA/(model.overview.caTTC||1)*100).toFixed(1)} %.`],hypotheses:[],actions:['Conserver cette zone comme témoin de comparaison et surveiller la pérennité de la hausse.'],entities:[positive.name]});
