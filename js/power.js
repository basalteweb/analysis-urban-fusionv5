...
AU.power = (() => {
  ...
  function bindProfileModal() {
    ...
    document.getElementById('saveStoreBtn')?.addEventListener('click', async () => {
      ...
      try {
        saveBtn.disabled = true; saveBtn.textContent = 'Analyse de l’adresse…';
        status.textContent = 'Géocodage officiel en cours…';
        const geo = await geocode(base.address);
        const p = saveProfile({ ...base, ...geo });
        status.textContent = `✓ ${p.geocodeLabel} · score ${(p.geocodeScore*100).toFixed(0)} %`;
      ...
      } finally { saveBtn.disabled = false; saveBtn.textContent = 'Enregistrer & analyser'; }
    });
    const p = loadProfile();
