...
AU.causalContext = (() => {
  ...
  function competingSignals(model,f,scope,windows){
    ...
    const agenda=agendaShift(model,windows);
    if(Math.abs(agenda.delta)>=3) out.push({key:'events',label:'Événements locaux',strength:clamp(40+Math.abs(agenda.delta)*4,40,78),evidence:`${agenda.current} événement(s) officiel(s) sur la fenêtre récente vs ${agenda.previous} précédemment.`});
    ...
    if(parking&&Math.abs(parking.delta)>=5) out.push({key:'parking',label:'Stationnement / accessibilité',strength:clamp(45+Math.abs(parking.delta)*1.7+(pc!==null&&pc!==undefined?Math.abs(pc)*20:0),45,85),evidence:`Occupation moyenne des parkings ${parking.current.toFixed(1)} % vs ${parking.previous.toFixed(1)} % ; ${parking.samplesCurrent+parking.samplesPrevious} relevés archivés.`});
    ...
    if(t2c&&Math.abs(t2c.delta)>=60) out.push({key:'t2c',label:'T2C / transports collectifs',strength:clamp(48+Math.abs(t2c.delta)/6+(tc!==null&&tc!==undefined?Math.abs(tc)*18:0),48,88),evidence:`Retard T2C moyen archivé ${Math.round(t2c.current)} s vs ${Math.round(t2c.previous)} s.`});
    else {const live=model.contextCorrelation?.urban?.t2c;if(live?.decoded&&((live.avgDelay||0)>=180||(live.cancelled||0)>0))out.push({key:'t2c-live',label:'T2C / transports collectifs',strength:58,evidence:`Contexte actuel : retard moyen ${Math.round(live.avgDelay||0)} s${live.cancelled?` · ${live.cancelled} annulation(s)`:''}. Historique insuffisant pour une attribution forte.`});}
    const cvelo=historyMetricShift(model,windows,'cvelo_bikes_available');
    const cc=model.contextCorrelation?.urban?.history?.cvelo_bikes_available?.visitsCorrelation;
    if(cvelo&&Math.abs(cvelo.delta)>=8) out.push({key:'cvelo',label:'C.vélo / mobilité de proximité',strength:clamp(34+Math.abs(cvelo.delta)*0.8+(cc!==null&&cc!==undefined?Math.abs(cc)*16:0),34,68),evidence:`Vélos C.vélo disponibles ${Math.round(cvelo.current)} vs ${Math.round(cvelo.previous)} sur les snapshots comparables. Signal de mobilité, pas preuve de fréquentation commerciale.`});
    const bike=model.contextCorrelation?.urban?.bike;
    if(bike?.visitsCorrelation!==null&&bike?.visitsCorrelation!==undefined&&Math.abs(bike.visitsCorrelation)>=0.25) out.push({key:'bike',label:'Activité urbaine / vélo',strength:clamp(35+Math.abs(bike.visitsCorrelation)*45,35,72),evidence:`Corrélation comptages vélo / visites ${bike.visitsCorrelation.toFixed(2)} sur ${bike.days} jour(s) appariés.`});
    if(f.category==='margin'){const d=model.intelligence?.findings?.find(x=>x.id==='discount-shift'); if(d) out.push({key:'discount',label:'Remises / mix de marge',strength:65,evidence:d.summary});}
  ...
  function testFinding(model,f){
    ...
    const externalEligible=new Set(['turnover','traffic','geo','customer','anomaly']).has(f.category);
    ...
    const retainedCauses=candidates.filter(c=>c.strength >= (c.type==='works'?68:60)).sort((a,b)=>b.strength-a.strength);
    ...
    const label=status==='strong'?'Explication fortement compatible':status==='moderate'?'Explication compatible':'Aucune cause suffisamment étayée';
    const chain=[];
    if(primary){
      chain.push(`Constat : ${f.title}`);
      chain.push(`Cause retenue : ${primary.label} (${Math.round(primary.strength)}/100).`);
      if(top) chain.push(`Zone contributrice : ${top.zone} (${Math.round(top.share*100)} % du mouvement géolocalisé).`);
      if(top?.excess!==null&&top?.excess!==undefined) chain.push(`Écart zone / reste de la clientèle : ${top.excess>=0?'+':''}${(top.excess*100).toFixed(1)} points.`);
      chain.push(`Preuve : ${primary.evidence}`);
      if(primary.work?.around) chain.push(`${direction<0?'Après démarrage':'Autour de la fin'} : CA zone ${primary.work.around.delta>=0?'+':''}${(primary.work.around.delta*100).toFixed(1)} %, visites ${primary.work.around.visitDelta>=0?'+':''}${(primary.work.around.visitDelta*100).toFixed(1)} % sur ${primary.work.around.days} jours.`);
    }

    const summary=primary
      ? `${primary.label} est la cause la mieux étayée pour ce constat. Analysis ne l’affiche qu’après convergence du signal métier, de la chronologie et des données disponibles.`
      : `Aucune cause testée n’atteint le niveau de preuve requis. Analysis n’affiche donc aucune explication causale dans le cockpit.`;
  ...
  function answer(model){
    ...
    return {
      ...
      intro:`${c.strong+c.moderate} diagnostic(s) disposent d’une explication suffisamment étayée sur ${c.tested} tests. Les hypothèses faibles ne sont pas affichées.`,
      items:c.top.slice(0,8).map(x=>({
        title:`${x.retainedCauses?.[0]?.label||x.label} · ${x.title}`,
        text:x.summary,
        bullets:x.chain.slice(1,6),
        confidence:`${x.score} %`,quality:'cause retenue'
      })),
